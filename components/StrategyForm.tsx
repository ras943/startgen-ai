...
// Define StrategyInput type (assumed to be in ../types)
...
// Define ToastContext type for useToasts
...
const CardHeader: React.FC<{ icon: string; title: string }> = ({ icon, title }) => (
  <div className="flex items-center gap-3 mb-6">
    <i className={`fas ${icon} text-[#6E44FF] text-xl`} aria-hidden="true"></i>
...
const FormGroup: React.FC<{
  ...
  <div className="mb-6">
    <label htmlFor={htmlFor} className="block mb-2 font-medium text-gray-300">
      {label}
    </label>
    {children}
    {error && (
      <div id={`${htmlFor}-error`} className="text-red-400 text-sm mt-2 flex items-center gap-2">
...
>(({ hasError, ...props }, ref) => (
  <input
    ...
    } rounded-xl text-gray-100 placeholder-gray-400 transition-all duration-300 focus:outline-none focus:border-[#6E44FF] focus:ring-2 focus:ring-[#6E44FF]/50`}
  />
));
...
>(({ hasError, children, ...props }, ref) => (
  <select
    ...
    className={`w-full p-3 bg-white/10 border ${
      hasError ? 'border-red-500' : 'border-white/10'
    } rounded-xl text-gray-100 transition-all duration-300 focus:outline-none focus:border-[#6E44FF] focus:ring-2 focus:ring-[#6E44FF]/50 appearance-none select-arrow`}
  >
    {children}
  </select>
));
...
const StrategyForm: React.FC<StrategyFormProps> = ({ onGenerate, isLoading }) => {
  ...
  const { addToast } = useToasts() as ToastContextType;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validate()) {
      ...
      setFormData({ topic: '', goal: '', audience: '', tone: 'Professional', framework: '' });
      addToast('Strategy submitted successfully!', 'success');
    } else {
      ...
      <form onSubmit={handleSubmit} noValidate>
        <FormGroup label="Topic *" htmlFor="topic" error={errors.topic}>
          <Input
            ...
            placeholder="e.g., Sustainable Fashion, Digital Marketing Trends"
        ...
        <div className="flex gap-4">
          ...
          <button
            type="button"
            onClick={handleReset}
            className="w-full p-4 bg-gray-600 text-white font-semibold rounded-xl hover:bg-gray-700 transition-all duration-300"
          >
            Reset Form
          </button>
        </div>
